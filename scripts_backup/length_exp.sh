...
-lr 2e-3 -visible_gpus 0,1,2,3 -report_every 50 -save_checkpoint_steps 2000 -batch_size 2000 -train_steps 50000 -accum_count 2 \
...
-model_name bert_lstm -pretrained_name $MODEL_DIR -max_model_pos=$MAX_MODEL_POS
cd ../../..
...
-sep_optim true -use_interval true -visible_gpus 0,1,2,3 \
-max_pos $MAX_POS -max_length 200 -alpha 0.95 -min_length 50 \
-result_path $BERT_OUTPUT_PATH/logs/exp_bert_cnndm.res \
-test_all true -model_name bert_lstm -pretrained_name $MODEL_DIR -max_model_pos=$MAX_MODEL_POS -temp_dir=$ROUGE_TEMP_DIR
cd ../../..
