...
mkdir $DATA_DIR
cp /data/zhuoyu/summarization/cnndm_data/bert_small_data.zip $DATA_DIR/bertsum.zip
cd $DATA_DIR
unzip bertsum.zip
cd ..
...
-lr 2e-3 -visible_gpus 0 -report_every 50 -save_checkpoint_steps 5 -batch_size 1000 -train_steps 11 -accum_count 1 \
...
-model_name bert_lstm -pretrained_name $MODEL_DIR -max_model_pos=128
cd ../../..
...
-test_all true -model_name bert_lstm -pretrained_name $MODEL_DIR -max_model_pos=128 -temp_dir=$ROUGE_TEMP_DIR
cd ../../..
