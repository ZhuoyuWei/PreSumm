...
class GlobalMCLSTMLayer(nn.Module):
    ...
    def forward(self, hidden_chunks):
        ...
        hidden_states_t= hidden_states.transpose(1,2).contiguous().view(ori_size[0]*ori_size[2],ori_size[1],ori_size[3]) # bl * c * d
        ...
        #print('debug output size = {}'.format(outputs.size()))
        return outputs
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None):
        ...
        outputs = []
        for i in range(len(output_chucks[0])):
            tmp_chunks=[]
...
class BertLSTMForPreTraining(BertLSTMPreTrainedModel):
    ...
    """
    def __init__(self, config):
        super(BertLSTMForPreTraining, self).__init__(config)

        self.bert = BertLSTMModel(config)
...
class BertLSTMForMaskedLM(BertLSTMPreTrainedModel):
    ...
    """
    def __init__(self, config):
        super(BertLSTMForMaskedLM, self).__init__(config)
...
class BertLSTMForNextSentencePrediction(BertLSTMPreTrainedModel):
    ...
    """
    def __init__(self, config):
        super(BertLSTMForNextSentencePrediction, self).__init__(config)
...
class BertLSTMForMultipleChoice(BertLSTMPreTrainedModel):
    ...
    """
    def __init__(self, config):
        super(BertLSTMForMultipleChoice, self).__init__(config)
...
class BertLSTMForTokenClassification(BertLSTMPreTrainedModel):
    ...
    """
    def __init__(self, config):
        super(BertLSTMForTokenClassification, self).__init__(config)
        self.num_labels = config.num_labels
...
class BertLSTMForQuestionAnswering(BertLSTMPreTrainedModel):
    ...
    """
    def __init__(self, config):
        super(BertLSTMForQuestionAnswering, self).__init__(config)
        self.num_labels = config.num_labels
